...
def VideoRead(video_path, startPoint, n_frames=5):
    cap = cv2.VideoCapture(video_path)
    cap.set(cv2.CAP_PROP_POS_FRAMES, start_frame)

    
    video_object = torchvision.io.VideoReader(video_path, 'video')
    # video_object.seek(0) returns an iterable of frames
    frames = itertools.islice(video_object.seek(startPoint), startPoint + n_frames)
