...
class CubePad(nn.Module):
    def __init__(self, lrtd_pad, use_gpu=False):
...
class CubePadding(nn.Module):
    ...
    def __init__(self, lrtd_pad, use_gpu=False):
        ...
        self.use_gpu = False
        # print(f"Specifically its now {self.use_gpu}")
        #self.pad = pad
        if type(lrtd_pad) == int:
            self.p_l = lrtd_pad
            self.p_r = lrtd_pad
    ...
    def flip(self, tensor, dim):
        ...
        # print(f"Before: {self.use_gpu}")
        self.use_gpu = False
        # print(f"After: {self.use_gpu}")
